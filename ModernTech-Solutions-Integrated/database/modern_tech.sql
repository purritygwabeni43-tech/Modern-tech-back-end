...
-- ModernTech Solutions - Integrated MySQL database
-- WARNING: This setup script drops and recreates the project tables.
CREATE DATABASE IF NOT EXISTS modern_tech2 CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE modern_tech2;
...
CREATE TABLE employees (
  ...
  CONSTRAINT fk_employee_department FOREIGN KEY (department_id) REFERENCES departments(department_id) ON UPDATE CASCADE ON DELETE RESTRICT,
  INDEX idx_employee_name(name), INDEX idx_employee_department(department_id)
) ENGINE=InnoDB;
...
CREATE TABLE users (
  ...
  CONSTRAINT fk_user_employee FOREIGN KEY(employee_id) REFERENCES employees(employee_id) ON UPDATE CASCADE ON DELETE SET NULL,
  CONSTRAINT fk_user_role FOREIGN KEY(role_id) REFERENCES roles(id) ON UPDATE CASCADE ON DELETE RESTRICT,
  INDEX idx_user_email(email), INDEX idx_user_username(username), INDEX idx_user_role(role_id)
) ENGINE=InnoDB;
...
CREATE TABLE attendance (
  ...
  CONSTRAINT fk_attendance_employee FOREIGN KEY(employee_id) REFERENCES employees(employee_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE leave_requests (
  ...
  CONSTRAINT fk_leave_employee FOREIGN KEY(employee_id) REFERENCES employees(employee_id) ON UPDATE CASCADE ON DELETE CASCADE,
  INDEX idx_leave_employee_date(employee_id,leave_date), INDEX idx_leave_status(status)
) ENGINE=InnoDB;
...
CREATE TABLE payroll (
  ...
  CONSTRAINT fk_payroll_employee FOREIGN KEY(employee_id) REFERENCES employees(employee_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE performance_reviews (
  ...
  CONSTRAINT fk_review_employee FOREIGN KEY(employee_id) REFERENCES employees(employee_id) ON UPDATE CASCADE ON DELETE CASCADE,
  INDEX idx_review_employee_date(employee_id,review_date)
) ENGINE=InnoDB;

INSERT INTO roles(role_name) VALUES ('HR'),('Manager'),('Employee');
...
-- Demo passwords are legacy plaintext only in this seed file for classroom/local setup.
-- The server automatically upgrades a successful plaintext login to bcrypt on first login.
INSERT INTO users(employee_id,full_name,email,username,password_hash,role_id,is_active) VALUES
(1,'Sibongile Nkosi','sibongile@moderntech.com','sibo.nkosi','Sibo123',(SELECT id FROM roles WHERE role_name='Employee'),1),
(5,'Zanele Khumalo','zanele@moderntech.com','zanele.khumalo','Zanele456',(SELECT id FROM roles WHERE role_name='Employee'),1),
(2,'Lungile Moyo','lungile@moderntech.com','Lungi.manager','Manager789',(SELECT id FROM roles WHERE role_name='Manager'),1),
(10,'Fatima Patel','fatima@moderntech.com','Fatima.admin','HR123',(SELECT id FROM roles WHERE role_name='HR'),1);
...
SELECT COUNT(*) AS leave_requests FROM leave_requests;
SELECT COUNT(*) AS payroll_records FROM payroll;
SELECT COUNT(*) AS performance_reviews FROM performance_reviews;
