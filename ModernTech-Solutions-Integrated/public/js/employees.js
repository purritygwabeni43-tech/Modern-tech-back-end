...
async function loadDepartments() {
    ...
    try {
        ...
        if (!response.ok || !Array.isArray(departments)) return;
        select.innerHTML = '<option value="" disabled selected>Select a Department...</option>' +
            departments.map(dept => `<option value="${dept.department_id}">${dept.department_name} (Dept ${dept.department_id})</option>`).join('');
    } catch (err) {
...
async function fetchEmployees(searchQuery = '') {
    try {
        const response = await fetch(`/employees?search=${encodeURIComponent(searchQuery)}`);
